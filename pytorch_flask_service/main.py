...
@app.route("/predict", methods=["POST"])#这是一个装饰器，它告诉 Flask，当有 POST 请求发送到 /predict 路径时，应该调用 predict 函数来处理这个请求。
@torch.no_grad()
def predict():
...
@app.route("/", methods=["GET", "POST"])#无论请求是 GET 还是 POST 类型，用户都会看到相同的页面。
def root():
    return render_template("up.html")


if __name__ == '__main__':
    app.run(host="0.0.0.0", port=5000)#允许来自任何IP地址的请求0.0.0.0,指定服务器要监听的端口号 5000 端口
